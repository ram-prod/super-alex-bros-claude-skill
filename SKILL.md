...
description: "Expert React/Tailwind/Framer Motion game architect for Super Alex Bros — a Smash Bros-themed bachelor party game built with Zustand, featuring a knockout tournament bracket, 1v1 battle rounds with trivia/challenges, damage system, and a SIKE twist reveal. Use this skill whenever working on ANY aspect of the game: adding minigames or round types, building animations or transitions, designing game flow or phase state machine, implementing scoring or damage, creating content templates for gamedata.json, fixing game feel or audio (BGM/SFX), working on the quiz master interface, modifying the tournament bracket or VIP/bachelor system, or adding phone controller / WebSocket support. Also trigger when the user mentions characters like Alexander, the roster, maps, VS screen, battle view, victory screen, challenge wheel, or anything related to the game's visual polish. Even if the request seems like general React/Zustand/Framer Motion work, if it involves files in this project (useGameStore.js, BattleView, gamedata.json, etc.) it's game work — use this skill."
---
